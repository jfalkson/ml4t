...
def get_portfolio_stats(port_val, daily_rf=0, samples_per_year=252):
    ...
    rf_rate = (daily_rf+1)**(1/samples_per_year) - 1
    
    avg_daily_ret = (daily_returns[1:]).mean()
    std_daily_ret = daily_returns[1:].std()
    sharpe_ratio = samples_per_year**.5 * (daily_returns[1:] - rf_rate).mean() / (daily_returns[1:]).std()
...
def plot_normalized_data(df, title="Normalized prices", xlabel="Date", ylabel="Normalized price"):
    ...
    ----------
        ...
        ylabel: Y-axis label
    """
    plot_data(df/df.ix[0], title=title, xlabel=xlabel, ylabel=ylabel)
